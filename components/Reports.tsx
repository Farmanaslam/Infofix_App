...
const Reports: React.FC = () => {
        ...
        const fetchReportData = async () => {
            ...
            try {
                ...
                if (ticketFilters.assignedTo && ticketFilters.assignedTo !== 'ALL') {
                    if (ticketFilters.assignedTo === 'UNASSIGNED') query = query.is('assigned_to', null);
                ...
                if (ticketFilters.warranty && ticketFilters.warranty !== 'ALL') query = query.eq('warranty', ticketFilters.warranty);
    ...
    const kpis = useMemo(() => {
        ...
        const avgResolutionTimeMs = resolvedTicketsInPeriod.length > 0 ?
        ...
        return {
            total: totalTickets,
            open: reportTickets.filter(t => t.status !== 'Resolved' && t.status !== 'SERVICE DONE').length,
            resolved: resolvedTicketsInPeriod.length,
            resolutionRate: totalTickets > 0 ? `${((resolvedTicketsInPeriod.length / totalTickets) * 100).toFixed(1)}%` : 'N/A',
            avgResolutionTime: `${(avgResolutionTimeMs / (1000 * 3600 * 24)).toFixed(1)} days`,
            highPriorityOpen: reportTickets.filter(t => t.priority.toUpperCase().includes('HIGH') && t.status !== 'Resolved' && t.status !== 'SERVICE DONE').length,
        };
    }, [reportTickets]);
    ...
    const generateInsights = async () => {
        ...
        if (!process.env.API_KEY || process.env.API_KEY === 'your_gemini_api_key_here') {
            setInsightsError("Gemini API Key is missing or invalid in environment variables.");
        ...
        try {
            ...
            const agentCounts = reportTickets.filter(t => t.resolvedAt).reduce((acc, t) => {
            ...
            setInsightsError(`An error occurred while generating insights: ${error.message}`);
        } finally {
            setIsLoadingInsights(false);
        }
    };
    ...
    useEffect(() => {
        ...
        const chartRefs = [
            ticketVolumeChartRef, statusChartRef, agentChartRef, 
            deviceTypeChartRef, priorityChartRef, storeChartRef
        ];
        ...
        if (ticketVolumeChartRef.current) {
            ...
            if (!isNaN(sDate.getTime()) && !isNaN(eDate.getTime())) {
                for (let d = new Date(sDate); d <= eDate; d.setDate(d.getDate() + 1)) {
                    dateMap.set(d.toISOString().split('T')[0], { created: 0, resolved: 0 });
                }
            }
            ...
            reportTickets.forEach(t => {
                ...
                else if (!startDate) { // If ALL time, allow dynamic keys
                     if(!dateMap.has(createdDate)) dateMap.set(createdDate, { created: 1, resolved: 0 });
                ...
                if (t.resolvedAt) {
                    ...
                    if (dateMap.has(resolvedDate)) dateMap.get(resolvedDate)!.resolved++;
                        ...
                        if(!dateMap.has(resolvedDate)) dateMap.set(resolvedDate, { created: 0, resolved: 1 });
            ...
            const createdData = sortedLabels.map(label => dateMap.get(label)!.created);
            const resolvedData = sortedLabels.map(label => dateMap.get(label)!.resolved);

            chartInstances.push(new Chart(ticketVolumeChartRef.current, {
                type: 'line',
                data: {
                    labels: sortedLabels.map(d => new Date(d).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })),
                    datasets: [
                        {
                            label: 'Created',
                            data: createdData,
                ...
                options: {
                    ...commonOptions,
                    plugins: { ...commonOptions.plugins, title: { ...commonOptions.plugins?.title, text: 'Ticket Volume Trends' } },
                    scales: {
        ...
        if (statusChartRef.current) {
            ...
            chartInstances.push(new Chart(statusChartRef.current, {
                type: 'doughnut',
                data: {
                ...
                options: {
                    ...commonOptions,
                    cutout: '70%',
                    plugins: { ...commonOptions.plugins, title: { ...commonOptions.plugins?.title, text: 'Status Distribution' }, legend: { position: 'right' } }
        ...
        if (agentChartRef.current) {
            ...
            chartInstances.push(new Chart(agentChartRef.current, {
                type: 'bar',
                data: {
                ...
                options: {
                    ...commonOptions,
                    plugins: { ...commonOptions.plugins, title: { ...commonOptions.plugins?.title, text: 'Performance by Agent' }, legend: { display: false } },
                    scales: { x: { grid: { display: false } }, y: { grid: { color: '#F3F4F6' }, border: { display: false }, ticks: { precision: 0 } } }
                }
            }));
        }
        ...
        if (deviceTypeChartRef.current) {
            ...
            chartInstances.push(new Chart(deviceTypeChartRef.current, {
                type: 'doughnut',
                data: {
                    labels: Object.keys(deviceCounts),
                    datasets: [{
                        data: Object.values(deviceCounts),
                        backgroundColor: ['#EC4899', '#3B82F6', '#F59E0B', '#10B981', '#8B5CF6', '#6366F1'],
                        borderWidth: 0
                    }]
                },
                options: {
                    ...commonOptions,
                    cutout: '70%',
                    plugins: { ...commonOptions.plugins, title: { ...commonOptions.plugins?.title, text: 'Device Breakdown' }, legend: { position: 'right' } }
                } as ChartOptions<'doughnut'>
            }));
        }
        ...
        if (priorityChartRef.current) {
            ...
            const colors = Object.keys(priorityCounts).map(p => {
                ...
                if (pUpper.includes('HIGH') || pUpper.includes('URGENT')) return '#EF4444';
                if (pUpper.includes('MEDIUM')) return '#F59E0B';
                return '#10B981';
            });

            chartInstances.push(new Chart(priorityChartRef.current, {
                type: 'doughnut',
                data: {
                ...
                options: {
                    ...commonOptions,
                    cutout: '70%',
                    plugins: { ...commonOptions.plugins, title: { ...commonOptions.plugins?.title, text: 'Priority Distribution' }, legend: { position: 'right' } }
                } as ChartOptions<'doughnut'>
            }));
        }
        ...
        if (storeChartRef.current) {
            ...
            chartInstances.push(new Chart(storeChartRef.current, {
                type: 'bar',
                data: {
                ...
                options: {
                    ...commonOptions,
                    plugins: { ...commonOptions.plugins, title: { ...commonOptions.plugins?.title, text: 'Volume by Store' }, legend: { display: false } },
                    scales: { x: { grid: { display: false } }, y: { grid: { color: '#F3F4F6' }, border: { display: false }, ticks: { precision: 0 } } }
                }
            }));
        }
    ...
    const inputStyles = "w-full p-2 border border-gray-300 rounded-lg bg-white text-black focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm";
        ...
        <div>
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Analyze service performance using filtered data.</p>
                </div>
                ...
                {activeFilters.length > 0 && (
                    <div className="flex flex-wrap gap-2 items-center bg-blue-50 border border-blue-100 p-2 rounded-lg max-w-full md:max-w-md">
                        <span className="text-xs font-bold text-blue-800 uppercase mr-1">Active Filters:</span>
                        {activeFilters.map((f, i) => (
                            <span key={i} className="px-2 py-1 bg-white text-blue-600 text-xs font-semibold rounded border border-blue-200 shadow-sm">
                                {f}
                            </span>
                        ))}
                    </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-8">
                <div className="flex flex-wrap items-end gap-6">
                    <div className="flex-grow" style={{minWidth: '200px'}}>
                        <label htmlFor="date-range-filter" className="block text-sm font-semibold text-gray-700 mb-2">Report Period</label>
                        <select id="date-range-filter" className={inputStyles} value={dateRangeFilter} onChange={e => setDateRangeFilter(e.target.value)}>
                     ...
                     {dateRangeFilter === 'CUSTOM' && (
                        <>
                            <div className="flex-grow" style={{minWidth: '200px'}}>
                                ...
                                <input type="date" id="startDate" value={startDate} onChange={e => setStartDate(e.target.value)} max={endDate} className={inputStyles}/>
                            </div>
                             <div className="flex-grow" style={{minWidth: '200px'}}>
                                <label htmlFor="endDate" className="block text-sm font-semibold text-gray-700 mb-2">End Date</label>
                                <input type="date" id="endDate" value={endDate} onChange={e => setEndDate(e.target.value)} min={startDate} className={inputStyles}/>
                            </div>
                        </>
                    )}
                </div>
            ...
            {loadingReports ? (
                ...
                <>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                        <KpiCard title="Total Tickets" value={kpis.total} color="bg-blue-50 text-blue-600" icon={<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" /></svg>} />
                        <KpiCard title="Open Tickets" value={kpis.open} color="bg-yellow-50 text-yellow-600" icon={<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
                        <KpiCard title="Resolved Tickets" value={kpis.resolved} color="bg-green-50 text-green-600" icon={<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
                        <KpiCard title="Resolution Rate" value={kpis.resolutionRate} color="bg-purple-50 text-purple-600" icon={<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" /></svg>} />
                        <KpiCard title="Avg. Resolution Time" value={kpis.avgResolutionTime} color="bg-indigo-50 text-indigo-600" icon={<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>} />
                        <KpiCard title="High Priority Open" value={kpis.highPriorityOpen} color="bg-red-50 text-red-600" icon={<svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>} />
                    </div>
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-blue-600 rounded-2xl p-8 shadow-lg text-white">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
                    <div>
                        <h3 className="text-2xl font-bold flex items-center">
                            <svg className="w-8 h-8 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                            AI Business Insights
                        </h3>
                        <p className="text-blue-100 mt-2 max-w-2xl">
                            Generate strategic recommendations based on your current filtered dataset ({kpis.total} tickets).
                        </p>
                    </div>
                    <button 
                ...
                {insightsError && (
                    <div className="bg-red-500/20 border border-red-400/30 p-4 rounded-lg text-white mb-4">
                        <p className="font-bold flex items-center"><svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>{insightsError}</p>
                    </div>
                )}
                ...
                {insights && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                        {insights.map((insight, index) => (
                            <div key={index} className="bg-white/10 backdrop-blur-sm p-5 rounded-xl border border-white/10 hover:bg-white/20 transition">
                                <div className="flex items-start">
                                    ...
                                    <p className="text-lg leading-relaxed" dangerouslySetInnerHTML={{ __html: insight.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>') }}></p>
                                </div>
                            </div>
                        ))}
                    </div>
