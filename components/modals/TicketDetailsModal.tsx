...
import { Customer, Ticket, AppSettings, TicketStatus, AuditLogEntry } from '../../types';
...
const TicketDetailsModal: React.FC<TicketDetailsModalProps> = ({ ticket, customer, settings, onSave, onClose, auditLog }) => {
    const { currentUser } = useStore();
    
    // Permission Checks
    const isTeamMember = currentUser?.type === 'team';
    const canEditRestrictedFields = isTeamMember && (currentUser.role === 'ADMIN' || currentUser.role === 'MANAGEMENT');
    ...
    const handleSave = async () => {
        ...
        const success = await onSave(formData, hasStoreChanged ? transferNote : undefined);
        if(success) onClose();
    };
    ...
    const downloadHistoryPdf = () => {
        ...
        doc.text(`Customer: ${customer?.name || 'Unknown'} (${customer?.phone || 'N/A'})`, 14, 35);
        
        // Include Brand Service in device details string
        const brandInfo = ticket.device.brand || ticket.device.brandService || '';
        doc.text(`Device: ${ticket.device.type} ${brandInfo} ${ticket.device.model || ''}`, 14, 40);
    ...
    const disabledInputClasses = "w-full p-2 border border-gray-300 rounded bg-gray-100 text-gray-600 cursor-not-allowed";

    // Helper to check for hold status robustly (handling case and whitespace)
    const isHoldStatus = (status: string) => status.trim().toUpperCase() === 'HOLD';
            ...
            <div className="bg-white p-0 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden text-black">
                ...
                <div className="flex border-b bg-white">
                    <button 
                        ...
                        className={`px-6 py-3 font-medium text-sm capitalize ${activeTab === 'details' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                        Details
                    </button>
                    <button 
                        onClick={() => setActiveTab('history')} 
                        className={`px-6 py-3 font-medium text-sm capitalize ${activeTab === 'history' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:bg-gray-50'}`}
                    >
                        History
                    </button>
                </div>
                        ...
                        <div className="space-y-4">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input value={formData.subject} onChange={e => setFormData({...formData, subject: e.target.value})} className={inputClasses} />
                                </div>
                                <div>
                                    ...
                                    <select 
                                        ...
                                        onChange={e => {
                                            ...
                                            setFormData(prev => ({
                                                ...
                                                holdReason: isHoldStatus(newStatus) ? prev.holdReason : undefined
                                            }));
                                        }} 
                                    ...
                                    <div className="animate-fade-in bg-orange-50 p-3 rounded-md border border-orange-200 md:col-span-2">
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            >
                                                ...
                                                {settings.holdReasons.map(r => <option key={r} value={r}>{r}</option>)}
                                            </select>
                                        </div>
                                ...
                                <div>
                                    ...
                                    <select value={formData.priority} onChange={e => setFormData({...formData, priority: e.target.value})} className={inputClasses}>
                                        {settings.priorities.map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                </div>
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className={canEditRestrictedFields ? inputClasses : disabledInputClasses}
                                        disabled={!canEditRestrictedFields}
                                        title={!canEditRestrictedFields ? "Only Admin and Management can change assignment" : "Assign to team member"}
                                    >
                                ...
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className={canEditRestrictedFields ? inputClasses : disabledInputClasses}
                                        disabled={!canEditRestrictedFields}
                                        title={!canEditRestrictedFields ? "Only Admin and Management can change store" : "Select Store"}
                                    >
                                ...
                                <div>
                                    ...
                                    <input type="number" value={formData.amountEstimate} onChange={e => setFormData({...formData, amountEstimate: parseFloat(e.target.value)})} className={inputClasses} />
                                </div>
                                <div>
                        ...
                        <div className="flex flex-col h-full">
                            ...
                            <ul className="space-y-4">
                                {history.map(log => {
                                    ...
                                    const hasTransferNote = log.details.includes(transferNotePrefix);
                                    let content;
                                        ...
                                        content = (
                                            <>
                                                ...
                                                <div className="mt-2 p-3 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-900 rounded-r-md">
                                                    <span className="font-bold text-yellow-700 text-xs uppercase tracking-wide block mb-1">Transfer Note</span>
                                                    {parts[1]}
                                                </div>
                                            </>
                                        );
                                    ...
                                    return (
                                        <li key={log.id} className="text-sm border-l-2 border-gray-300 pl-4">
                                            <p className="font-bold">{log.user} <span className="font-normal text-gray-500 text-xs">{new Date(log.timestamp).toLocaleString()}</span></p>
                                            <div className="text-gray-700 mt-1">{content}</div>
                                        </li>
                                    );
                                })}
                                {history.length === 0 && <p className="text-gray-500 italic">No history available for this ticket.</p>}
                            </ul>
                        </div>
                    )}
                </div>
                ...
                <div className="p-4 border-t bg-gray-50 flex justify-end gap-2">
                    <button onClick={onClose} className="px-4 py-2 bg-white border rounded text-black hover:bg-gray-100">Cancel</button>
