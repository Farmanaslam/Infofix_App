...
const Schedule: React.FC = () => {
  ...
  const tasksByDate = useMemo(() => {
    const isTechnician =
      currentUser?.type === "team" && currentUser.role === "TECHNICIAN";
  ...
  const ticketsByDate = useMemo(() => {
    return scheduledTickets.reduce((acc, ticket) => {
      if (ticket.scheduledDate) {
        (acc[ticket.scheduledDate] = acc[ticket.scheduledDate] || []).push(
          ticket
        );
      }
      return acc;
  ...
  const handleAddTask = async () => {
    ...
    let assignee: String[] | null =
      newTaskAssignee.length > 0 ? newTaskAssignee : null;
    ...
    try {
      ...
      if (error) {
        ...
        if (
          msg.includes("assigned_to") ||
          msg.includes("schema cache") ||
          msg.includes(
            'column "assigned_to" of relation "tasks" does not exist'
          )
        ) {
          console.warn(
            "Database schema outdated: 'assigned_to' column missing. Retrying without assignment."
          );
          ...
          if (retry.error) {
            ...
            setTasks((current) =>
              current.map((t) =>
                t.id === newTask.id ? { ...t, assignedTo: undefined } : t
              )
            );
            // Exact message matching user report
            toast(
              "Task added. Note: Assignment not saved (Database update required).",
  ...
  const renderCalendar = () => {
    ...
    const days = Array.from({ length: firstDayOfMonth }, (_, i) => (
      <div
        key={`empty-${i}`}
        className="border-t border-r border-gray-200"
      ></div>
    ));
        ...
        >
          ...
          <div className="absolute bottom-1 right-1 flex gap-1">
            {ticketsForDay.length > 0 && (
              <span
                className={`text-[10px] md:text-xs font-semibold px-1 md:px-1.5 py-0.5 rounded-full flex items-center gap-0.5 md:gap-1 ${
                  isSelected
            ...
            {tasksForDay.length > 0 && (
              <span
                className={`text-[10px] md:text-xs font-semibold px-1 md:px-1.5 py-0.5 rounded-full ${
                  isSelected
                    ? "bg-white text-blue-600"
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow-md h-full flex flex-col">
        ...
        <div className="grid grid-cols-7 flex-grow border-l border-b border-gray-200">
          {days}
        </div>
        <button
          onClick={goToToday}
          className="mt-4 w-full py-2 bg-gray-200 text-black font-semibold rounded-md hover:bg-gray-300 transition"
        >
          Today
        </button>
      </div>
              ...
              <div className="space-y-3">
                {selectedDayTasks.length > 0 ? (
                  selectedDayTasks.map((task) => (
                    <div
                      key={task.id}
                      className={`relative overflow-hidden bg-white border border-gray-200 rounded-xl p-4 shadow-sm group transition-all duration-300 hover:shadow-md ${
                      ...
                      <div className="flex items-start gap-3 relative z-10">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-wrap justify-between items-start gap-2">
                            <span
                              className={`font-medium text-sm transition-colors break-words ${
                                task.completed
              ...
              <div className="mt-4 flex flex-col space-y-2">
                ...
                <div className="flex items-start gap-2 w-full">
                  {canAssignTasks && settings && (
                    <div className="relative flex-1 min-w-0">
                      <div
                        ...
                        className="h-[38px] max-w-full px-2 py-1 border border-gray-300 rounded-md bg-white text-sm cursor-pointer flex items-center gap-1 overflow-x-auto whitespace-nowrap focus-within:ring-2 focus-within:ring-blue-500"
                      >
                        {newTaskAssignee.length === 0 && (
                          <span className="text-gray-400">
                            Assign members (optional)
                          </span>
                        )}

                        {newTaskAssignee.map((member) => (
                          <span
                            key={member}
                      ...
                      {assigneeOpen && (
                        <div className="absolute z-30 mt-1 w-full bg-white border border-gray-200 rounded-md shadow-lg max-h-48 overflow-y-auto">
                          ...
                          {settings?.teamMembers?.map((m) => {
                            const selected = newTaskAssignee.includes(m.name);
                  ...
                  <button
                    ...
                    className="h-[38px] bg-blue-600 text-white px-4 rounded-md hover:bg-blue-700 font-semibold flex-shrink-0 whitespace-nowrap"
                  >
                    Add Task
                  </button>
                </div>
              ...
              <div className="space-y-2">
                {selectedDayTickets.length > 0 ? (
                  selectedDayTickets.map((ticket) => (
                    <div
                      key={ticket.id}
                      className="p-3 bg-blue-50 border-l-4 border-blue-500 rounded-r-lg shadow-sm hover:shadow-md transition-shadow"
                    >
                      ...
                      <div className="flex justify-between items-center mt-1">
                        <p className="text-xs text-gray-600">
                          ...
                          |{" "}
                          {ticket.customerName ||
                            customerMap.get(ticket.customerId)?.name ||
                            "Unknown"}
                        </p>
                  ...
                  <p className="text-gray-500 text-sm p-3 bg-gray-50 rounded-lg text-center border border-dashed border-gray-200">
