...
const ModernStatCard: React.FC<{
  ...
  >
    <div
      className={`absolute top-0 right-0 w-24 h-24 bg-gradient-to-br ${colorClass} opacity-10 rounded-full blur-2xl transform translate-x-8 -translate-y-8 group-hover:scale-150 transition-transform duration-500`}
    ></div>
    ...
    <div className="relative z-10 flex justify-between items-start">
      ...
      <div
        className={`p-3 rounded-xl bg-gradient-to-br ${colorClass} text-white shadow-lg group-hover:shadow-xl group-hover:scale-110 transition-all duration-300`}
      >
        {icon}
      </div>
    </div>
...
const Dashboard: React.FC = () => {
  ...
  const getPriorityBadgeClass = (priority: string) => {
    ...
    if (priorityUpper.includes("HIGH"))
      return "bg-red-100 text-red-700 border border-red-200 ring-1 ring-red-200";
    if (priorityUpper.includes("MEDIUM"))
      return "bg-orange-100 text-orange-700 border border-orange-200 ring-1 ring-orange-200";
    if (priorityUpper.includes("LOW"))
      return "bg-green-100 text-green-700 border border-green-200 ring-1 ring-green-200";
  ...
  const handleStatClick = (
    type: "open" | "overdue" | "resolved" | "customers"|"taskAssigned"
  ) => {
    if (type === "customers") {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1 flex items-center">
            ...
            <span className="font-medium text-gray-700 ml-1 mr-1">
              {weekday},
            </span>{" "}
            {fullDate}
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          ...
          icon={
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                strokeWidth="2"
                d="M7 16V4m0 12h10m0-12v12m-5 4h.01M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"
              />
            </svg>
          }
        ...
        />
         <ModernStatCard
          title="Task Assigned"
          value={dashboardStats.taskAssigned}
          icon={
            <svg
              className="w-6 h-6"
              fill="none"
          ...
          onClick={() => handleStatClick("taskAssigned")}
          colorClass="from-purple-500 to-violet-600"
          bgClass="bg-purple-50"
        />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <tbody className="divide-y divide-gray-100">
                {highPriorityTickets.map((ticket) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="font-bold text-gray-800 text-sm mb-1 group-hover:text-blue-600 transition-colors">
                        {ticket.subject}
                      </div>
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold uppercase ${getPriorityBadgeClass(
                            ticket.priority
                          )}`}
                        >
                          {ticket.priority}
                  ...
                  <tr>
                    <td colSpan={3} className="p-10 text-center">
                      <div className="flex flex-col items-center justify-center text-gray-400">
                        <svg
                          className="w-12 h-12 mb-3 text-green-100"
                          fill="currentColor"
                          viewBox="0 0 20 20"
                        >
                          <path
                        ...
                        <p className="text-gray-500 font-medium">
                          All clear! No high priority tickets.
                        </p>
                      </div>
                    </td>
                  </tr>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            {(Object.entries(storeStats) as [string, StoreStat][])
                  ...
                  <div className="relative pt-1">
                    <div className="flex mb-2 items-center justify-between text-xs">
                      ...
                      <span className="text-blue-600 font-bold">
                        {Math.round(
                          (data.total / Math.max(1, openTickets)) * 100
                        )}
                        %
                      </span>
                    </div>
