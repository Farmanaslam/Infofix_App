...
const Tickets: React.FC = () => {
    const { 
        tickets, ticketsCount, setTickets,
        customers, settings, auditLog, setAuditLog, currentUser, addAuditLog,
    ...
    const activeFiltersCount = [
        ...
        ticketFilters.assignedTo !== 'ALL' && ticketFilters.assignedTo !== currentUser?.name, // Don't count My Tickets toggle here
        ticketFilters.deviceType !== 'ALL',
        ticketFilters.warranty !== 'ALL',
        ticketFilters.dateRange !== 'ALL'
    ].filter(Boolean).length;

    const isMyTicketsActive = currentUser?.type === 'team' && ticketFilters.assignedTo === currentUser.name;
    ...
    const handleUpdateTicket = async (updatedTicket: Ticket, transferNote?: string): Promise<boolean> => {
        ...
        if (updatedTicket.status === 'Resolved' && oldTicket?.status !== 'Resolved') {
        ...
        // Optimistic update
        setTickets(current => current.map(t => t.id === updatedTicket.id ? optimisticTicket : t));
        setSelectedTicket(optimisticTicket);

        const changes = [];
        if (oldTicket) {
            if (oldTicket.status !== updatedTicket.status) changes.push(`status to ${updatedTicket.status}`);
            if (oldTicket.priority !== updatedTicket.priority) changes.push(`priority to ${updatedTicket.priority}`);
            if (oldTicket.assignedTo !== updatedTicket.assignedTo) changes.push(`assignment to ${updatedTicket.assignedTo || 'Unassigned'}`);
            if (oldTicket.store !== updatedTicket.store) changes.push(`store to ${updatedTicket.store}`);
            if (oldTicket.amountEstimate !== updatedTicket.amountEstimate) changes.push(`estimate to ${updatedTicket.amountEstimate}`);
        }
    ...
    const getPriorityBadgeClass = (priority: string) => {
        ...
        if (p.includes('HIGH') || p.includes('URGENT')) return 'bg-red-50 text-red-700 border border-red-100';
        if (p.includes('MEDIUM')) return 'bg-orange-50 text-orange-700 border border-orange-100';
        return 'bg-green-50 text-green-700 border border-green-100';
    };

    const inputStyles = "w-full p-2 border border-gray-300 rounded-md text-sm bg-white text-black focus:outline-none focus:ring-2 focus:ring-blue-500 transition-shadow";
        ...
        <div>
                ...
                <div className="p-4 flex flex-col lg:flex-row gap-4 items-center justify-between bg-gray-50/50">
                    ...
                    <div className="flex w-full lg:w-auto gap-3 items-center justify-end flex-wrap md:flex-nowrap">
                        ...
                        {currentUser?.type === 'team' && (
                             ...
                             >
                                 <svg className={`w-4 h-4 mr-2 ${isMyTicketsActive ? 'text-blue-600' : 'text-gray-500'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                 My Tickets
                             </button>
                        )}
                        ...
                        >
                            ...
                            <svg className={`w-4 h-4 ml-2 transition-transform duration-200 ${filtersExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                        </button>
                    </div>
                </div>
                
                {/* Collapsible Filter Drawer */}
                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${filtersExpanded ? 'max-h-[500px] opacity-100 border-t border-gray-200' : 'max-h-0 opacity-0'}`}>
                    <div className="p-5 bg-gray-50">
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Status</label>
                                <select className={inputStyles} value={ticketFilters.status || 'ALL'} onChange={e => handleFilterChange('status', e.target.value)}>
                            ...
                            <div>
                                ...
                                <select className={inputStyles} value={ticketFilters.priority || 'ALL'} onChange={e => handleFilterChange('priority', e.target.value)}>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Assigned Agent</label>
                                <select className={inputStyles} value={ticketFilters.assignedTo || 'ALL'} onChange={e => handleFilterChange('assignedTo', e.target.value)}>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Store Location</label>
                                <select className={inputStyles} value={ticketFilters.store || 'ALL'} onChange={e => handleFilterChange('store', e.target.value)}>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Device Type</label>
                                <select className={inputStyles} value={ticketFilters.deviceType || 'ALL'} onChange={e => handleFilterChange('deviceType', e.target.value)}>
                            ...
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Warranty</label>
                                <select className={inputStyles} value={ticketFilters.warranty || 'ALL'} onChange={e => handleFilterChange('warranty', e.target.value)}>
                        ...
                        <div className="flex flex-col md:flex-row gap-6 pt-4 border-t border-gray-200 mt-2">
                            <div className="flex-1 flex flex-col md:flex-row gap-4 items-end">
                                <div className="w-full md:w-48">
                                    ...
                                    <select className={inputStyles} value={ticketFilters.dateRange || 'ALL'} onChange={e => handleFilterChange('dateRange', e.target.value)}>
                                ...
                                {ticketFilters.dateRange === 'CUSTOM' && (
                                    <div className="flex gap-2 items-center w-full md:w-auto bg-white p-1 rounded-md border border-gray-300">
                                        <input type="date" className="p-1 text-sm focus:outline-none" value={ticketFilters.startDate || ''} onChange={e => handleFilterChange('startDate', e.target.value)} />
                                        <span className="text-gray-400">-</span>
                                        <input type="date" className="p-1 text-sm focus:outline-none" value={ticketFilters.endDate || ''} onChange={e => handleFilterChange('endDate', e.target.value)} />
                                    </div>
                                )}
                            </div>
                ...
                {ticketsLoading ? (
                    ...
                    <>
                        {tickets.map(ticket => {
                            ...
                            const customerDisplayName = ticket.customerName || customers.find(c => c.id === ticket.customerId)?.name || ticket.customerId;
                            return (
                                <div 
                                    key={ticket.id} 
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="font-mono text-blue-600 font-bold text-sm">{ticket.id}</span>
                                    ...
                                    <div className="mb-2">
                                        <h3 className="font-bold text-gray-900 leading-tight">{ticket.subject}</h3>
                                        <p className="text-sm text-gray-600 mt-1">Customer: {customerDisplayName}</p>
                                    </div>
                                    ...
                                    {isExpanded && (
                                        <div className="mt-4 pt-3 border-t border-gray-100 animate-fade-in">
                                            <div className="grid grid-cols-2 gap-y-2 text-sm text-gray-700 mb-4">
                                                <div><span className="text-gray-500 text-xs block">Priority</span>
                                                    <span className={`inline-block px-2 py-0.5 rounded text-xs ${getPriorityBadgeClass(ticket.priority)}`}>{ticket.priority}</span>
                                                </div>
                                                <div><span className="text-gray-500 text-xs block">Device</span> {ticket.device?.type}</div>
                                                <div><span className="text-gray-500 text-xs block">Created</span> {new Date(ticket.createdAt).toLocaleDateString()}</div>
                                                <div><span className="text-gray-500 text-xs block">Store</span> {ticket.store}</div>
                                                {ticket.resolvedAt && <div><span className="text-gray-500 text-xs block">Resolved</span> {new Date(ticket.resolvedAt).toLocaleDateString()}</div>}
                                                <div className="col-span-2"><span className="text-gray-500 text-xs block">Assigned To</span> {ticket.assignedTo || 'Unassigned'}</div>
                                            </div>
                                            ...
                                            <div className="flex space-x-3 mt-2">
                                                ...
                                                {canDelete && (
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); setTicketToDelete(ticket); }}
            ...
            <div className="hidden md:block bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                {ticketsLoading ? (
                    <SkeletonTable 
                        rows={10} 
                        columnWidths={['w-16', 'w-48', 'w-32', 'w-24', 'w-24', 'w-32', 'w-32', 'w-16']} 
                    />
                ) : (
                                ...
                                {tickets.map(ticket => {
                                    ...
                                    <tr key={ticket.id} className="hover:bg-blue-50 transition-colors">
                                        ...
                                        <td className="p-4 font-medium text-gray-900">
                                            {ticket.subject}
                                            <div className="text-xs text-gray-500 mt-1">{ticket.device?.type} {ticket.device?.model ? `- ${ticket.device.model}` : ''}</div>
                                        </td>
                                        <td className="p-4">
                                            <button 
                                        ...
                                        <td className="p-4">
                                            <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${getStatusBadgeClass(ticket.status)}`}>
                                                {ticket.status}
                                            </span>
                                        </td>
                                        <td className="p-4">
                                            <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${getPriorityBadgeClass(ticket.priority)}`}>
                                                {ticket.priority}
                                            </span>
                                        </td>
                                        <td className="p-4 text-gray-600 whitespace-nowrap">
                                            <div title={`Created: ${new Date(ticket.createdAt).toLocaleString()}`}>
                                                {new Date(ticket.createdAt).toLocaleDateString()}
                                                <div className="text-xs text-gray-400">{new Date(ticket.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</div>
                                            </div>
                                            {ticket.resolvedAt && (ticket.status === 'Resolved' || ticket.status === 'SERVICE DONE') && (
                                                <div className="text-xs text-green-600 mt-1 font-medium" title={`Resolved: ${new Date(ticket.resolvedAt).toLocaleString()}`}>
                                                    Res: {new Date(ticket.resolvedAt).toLocaleDateString()}
                                                </div>
                                            )}
                                        </td>
                                        ...
                                        <td className="p-4 text-right">
                                            <div className="flex justify-end items-center space-x-2">
                                                <button onClick={() => openTicketDetails(ticket)} className="p-1 text-gray-400 hover:text-blue-600 transition" title="View Details">
                                                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                </button>
                                                {canDelete && (
                                                    <button onClick={() => setTicketToDelete(ticket)} className="p-1 text-gray-400 hover:text-red-600 transition" title="Delete Ticket">
                                                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.143A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.857L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                    </button>
                                                )}
                                            </div>
