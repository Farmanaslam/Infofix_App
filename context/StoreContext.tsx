...
const mapSupabaseToSettings = (s: any): AppSettings => ({
  id: s.id,
  teamMembers: Array.isArray(s.team_members)
    ? (s.team_members as any[]).map((m) => {
  ...
  holdReasons: Array.isArray(s.hold_reasons)
    ? (s.hold_reasons as string[])
    : [],
  priorities: Array.isArray(s.priorities) ? (s.priorities as string[]) : [],
  statuses: Array.isArray(s.statuses) ? (s.statuses as string[]) : [],
  pastDueDays:
    typeof s.past_due_days === "object" && s.past_due_days !== null
      ? (s.past_due_days as { [key: string]: number })
      : { HIGH: 3, MEDIUM: 7, LOW: 14 },
  deviceTypes: Array.isArray(s.device_types)
    ? (s.device_types as string[])
    : ["LAPTOP", "DESKTOP", "ACCESSORY", "CCTV", "BRAND SERVICE", "OTHER"],
  internalProgressReasons: Array.isArray(s.internal_progress_reasons)
    ? (s.internal_progress_reasons as string[])
    : [],
});
...
interface StoreContextType {
  ...
  logout: () => void;
  addAuditLog: (
    log: Omit<AuditLogEntry, "id" | "timestamp" | "user">
  ) => Promise<void>;

  // PWA Install Prompt
  installPrompt: BeforeInstallPromptEvent | null;
  setInstallPrompt: React.Dispatch<
...
export const StoreProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const fetchPaginatedTickets = async () => {
    ...
    if (ticketFilters.dateRange && ticketFilters.dateRange !== "ALL") {
      ...
      if (ticketFilters.dateRange === "TODAY")
        start = new Date(today.setHours(0, 0, 0, 0));
      else if (ticketFilters.dateRange === "LAST_7_DAYS")
        start = new Date(today.setDate(today.getDate() - 7));
      else if (ticketFilters.dateRange === "LAST_30_DAYS")
  ...
  const fetchPaginatedCustomers = async () => {
    ...
    if (customerSearch)
      query = query.or(
        `name.ilike.%${customerSearch}%,email.ilike.%${customerSearch}%,phone.ilike.%${customerSearch}%`
      );
  ...
  const fetchDashboardStats = async (currentUser: CurrentUser | null) => {
    if (!supabase) return null;
    ...
    const { count: openCount } = await supabase
      ...
      .not("status", "in", '("Resolved","SERVICE DONE")');
    ...
    const { data: storeRaw } = await supabase
      .from("tickets")
      .select("store, priority")
      .not("status", "in", '("Resolved","SERVICE DONE")');

    const storeStats: any = {};
    (storeRaw || []).forEach((t: any) => {
      if (!storeStats[t.store])
        storeStats[t.store] = { total: 0, priorities: {} };
      storeStats[t.store].total++;
      storeStats[t.store].priorities[t.priority] =
        (storeStats[t.store].priorities[t.priority] || 0) + 1;
    });
    ...
    if (currentUser?.type === "team") {
      ...
      if (!error && taskData) {
        taskAssignedCount = taskData.filter((task: any) => {
          if (task.completed) return false;
          if (!Array.isArray(task.assigned_to)) return false;
          return task.assigned_to.includes(currentUser.name);
        }).length;
      }
    }
    ...
    return {
      ...
      highPriorityTickets: (highPriority || []).map(mapSupabaseToTicket),
      storeStats,
      taskAssigned: taskAssignedCount, // ✅ fixed
    };
  };
  ...
  const ticketsQuery = useQuery({
    queryKey: ["tickets", ticketPage, ticketPageSize, ticketFilters],
    queryFn: fetchPaginatedTickets,
    enabled: isSupabaseConfigured,
    placeholderData: keepPreviousData,
  });
  const customersQuery = useQuery({
    queryKey: ["customers", customerPage, customerPageSize, customerSearch],
  ...
  const tasksQuery = useQuery({
    queryKey: [
      "tasks",
      currentUser?.type === "team" ? currentUser.name : "",
      currentUser?.type === "team" ? currentUser.role : "",
    ],
    queryFn: fetchTasks,
  ...
  const dashboardQuery = useQuery({
    ...
    queryFn: () => fetchDashboardStats(currentUser),
    enabled: isSupabaseConfigured && !!currentUser,
  });
  ...
  const scheduledTicketsQuery = useQuery({
    queryKey: ["scheduledTickets"],
    queryFn: fetchScheduledTickets,
    enabled: isSupabaseConfigured,
  });
  ...
  useEffect(() => {
    if (ticketsQuery.data) {
      ...
      setTicketsCount(ticketsQuery.data.count);
    }
  }, [ticketsQuery.data]);
  useEffect(() => {
    if (customersQuery.data) {
      setCustomers(customersQuery.data.data);
      setCustomersCount(customersQuery.data.count);
    }
  }, [customersQuery.data]);
  useEffect(() => {
  ...
  useEffect(() => {
    if (auditLogQuery.data) setAuditLog(auditLogQuery.data);
  }, [auditLogQuery.data]);

  useEffect(() => {
    if (dashboardQuery.data) setDashboardStats(dashboardQuery.data);
  }, [dashboardQuery.data]);
  useEffect(() => {
    if (pendingTicketsQuery.data) setPendingTickets(pendingTicketsQuery.data);
  }, [pendingTicketsQuery.data]);
  useEffect(() => {
    if (scheduledTicketsQuery.data)
      setScheduledTickets(scheduledTicketsQuery.data);
  }, [scheduledTicketsQuery.data]);
            ...
            if (payload.eventType === "UPDATE") {
              setTickets((current) =>
                current.map((t) =>
                  t.id === payload.new.id
                    ? {
                        ...mapSupabaseToTicket(payload.new),
          ...
          try {
            if (payload.eventType === "UPDATE") {
              setCustomers((current) =>
                current.map((c) =>
                  c.id === payload.new.id
                    ? mapSupabaseToCustomer(payload.new)
                    : c
                )
              );
            } else if (payload.eventType === "DELETE") {
